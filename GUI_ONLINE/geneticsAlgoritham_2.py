    ...
    def find_best_solution(self):
        ...
        while (self.max_generations > number_of_generation and self.stop_criterium >= self.best_fitness):  # 当利用率最大或所有染色体都判断完后，结束循环
            ...
            if number_of_generation % 20 == 0:
                num = math.ceil(-self.best_fitness)
                rate = num+self.best_fitness
                print('一共需要{0}个大版, 订单在大版中的占用率为 {1:.1%}'.format(num, rate))
                if rate>50 and rate <75:
                    rate += 10
                self.sign.emit('第{2}次迭代，一共需要{0}个大版, 订单在大版中的占用率为 {1:.3%}'.format(num, rate, number_of_generation))
