...
class item:
    def __init__(self, location,file_name,logistics_route,custom, amount,kuansu,order_num,cutedhg, size_x=0, size_y=0, page=0,
                 rgb=[-1, -1, -1], color_cluster=-1, character=0, models=1):
        # self.size_x = size_x
        # self.size_y = size_y
        self.location = location  # 订单存储的位置
        self.file_name = file_name  #  订单的名字
        ...
        self.logistics_route = logistics_route  # 快递专线
        self.custom = custom  # 客户
        self.amount = amount  # 打印数量
        self.kuansu = kuansu  # 款数
        self.order_num = order_num  # 订单序列
        self.cutedhg = cutedhg  # 订单特殊需求


    def copy_item(self):
        a = item(self.location,self.file_name,self.logistics_route,self.custom ,self.amount,self.kuansu,self.order_num,self.cutedhg, self.size_x, self.size_y, self.page, self.rgb, self.color_cluster)
        return a
